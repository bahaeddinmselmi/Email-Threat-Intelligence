// Outlook Content Script - Monitor and analyze emails in Outlook
(function() {
  'use strict';
   ...
   */
  function init() {
    Helpers.log('info', 'Initializing Outlook Email Threat Intelligence');
    
    emailParser = new EmailParser('outlook');
    uiInjector = new UIInjector('outlook');
  ...
  /**
   * Wait for Outlook interface to fully load
   */
  function waitForOutlookLoad() {
    return Helpers.waitForElement('div[role="region"]', 15000);
  }
  ...
  function setupEmailObserver() {
    ...
    // Listen for clicks on email items
    document.addEventListener('click', (e) => {
      const emailItem = e.target.closest('[role="listitem"]') || 
                       e.target.closest('[data-convid]');
      if (emailItem) {
  ...
  /**
   * Handle URL changes in Outlook
   */
  function handleUrlChange(url) {
    Helpers.log('info', 'Outlook URL changed', url);
    
    // Check if viewing an email
    if (url.includes('/mail/') || url.includes('/id/')) {
      setTimeout(() => checkCurrentEmail(), 1000);
    } else {
